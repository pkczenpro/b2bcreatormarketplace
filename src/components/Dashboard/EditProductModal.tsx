...
export default function EditProductModal({
  ...
  const handleSubmit = async () => {
    ...
    if (filesToDelete.productImages?.length) {
      formDataToSend.append(
        ...
        formData.productImages.filter(
          (file) => !filesToDelete.productImages?.includes(file)
        )
      );
    }
    if (filesToDelete.resources?.length) {
      formDataToSend.append(
        "resources",
        formData.resources.filter(
          (file) => !filesToDelete.resources?.includes(file)
        )
      );
    }
  ...
  const handleDeleteFile = (
    type: "productLogo" | "productImages" | "resources",
    fileUrl?: string,
    index?: number
  ) => {
                          ...
                          <div key={index} className="relative group">
                            ...
                            <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                              <Button
                                ...
                                danger
                                onClick={() =>
                                  handleDeleteFile("resources", resource, index)
                                }
                              >
                                Remove
                              </Button>
