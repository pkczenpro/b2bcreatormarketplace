...
const Template_2: React.FC<Template1Props> = ({
    index,
    isLastItem = false,
}) => {
    ...
    return (
        <div
            key={index}
            id={`post-${index}`}
            className="bg-[#7CA3C2] w-full h-full flex flex-col justify-between p-12 relative"
        >
            {/* Gradient images with z-index 0 */}
            <img src="/templates/temp2_pattern.svg" alt=""
                className="absolute top-0 right-0 w-[30%] object-cover z-0"
            />

            <img src="/templates/temp2_pattern2.svg" alt=""
                className="absolute bottom-0 left-0 w-[80%] object-cover z-0"
            />

            {/* Top Section - Text Content */}
            <div className={` z-10 relative ${index === 0 ? "text-left" : "text-center mt-32"}`}>
            ...
            < div className="flex justify-between items-center w-full z-10 relative" >
                ...
                < div className="flex items-center space-x-4" >
                    ...
                    <div>
                        <h1
                            ...
                            className="text-black text-left font-semibold text-lg">
                            {editableProfileName}
                        </h1>
                        <p
                            ...
                            className="text-sm text-black text-left">{editableProfileUsername}</p>
                    </div>
                </div>
                ...
                {
                    !isLastItem ? (
                        <ArrowRightCircle className="text-black w-7 h-7" />
                    ) :
                        <Bookmark className="text-black w-7 h-7" fill="black" />
                }
            </div >
        </div >
    );
