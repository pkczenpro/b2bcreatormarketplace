...
export function CreatorTable({ campaign, Refresh }) {
  ...
  const postReview = async () => {
    try {
      await api.post(
        `/campaigns/${campaign._id}/creators/${selectedCreator._id}/rate`,
        reviewState
      );
    ...
    >
      <div className="flex items-center gap-4 bg-neutral-50 p-2 px-4 rounded-xl">
        <img
          ...
          loading="lazy"
          src={
            selectedCreator?.profilePicture?.includes("http")
      ...
      <div className="mt-4">
        <p className="text-neutral-900 text-md mb-2">Rating</p>
        <Rate
          count={10}
      ...
      <div className="mt-4">
        <p className="text-neutral-900 text-md mb-2">Label</p>
        <Input.TextArea
  ...
  const acceptCreatorRequest = async (id: string) => {
    ...
    try {
      ...
      toast.success(
        "Creator request accepted successfully, content shared on LinkedIn",
        {
          position: "top-right",
        }
      );
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        ...
        >
          <img
            src={
              record?.profilePicture?.includes("http")
    ...
    },
    {
      ...
      render: (_, record) => {
        const isPending =
          record.status === "pending" || record.status === "prospect";
            ...
            {isDone && isAmountSet && (
              <Button
                onClick={() => {
                  if (record?.amount) {
                    router.push(
                      `/dashboard/pay-creator/${campaign._id}?creator=${record._id}&invoiceId=${invoiceId}&name=${record.name}`
                    );
                  } else {
              ...
              items: [
                {
                  ...
                  onClick: () => {
                    window.open(
                      `/dashboard/user-preview/${record._id}`,
                      "_blank"
                    );
                  },
                },
  ...
  const acceptWork = async (id: string) => {
    ...
    try {
      await api.post(
        `/campaigns/${campaign._id}/creators/${contentData._id}/accept`,
        {
          contentId: id,
        }
      );
  ...
  const giveAmount = async () => {
    ...
    try {
      await api.put(
        `/campaigns/${campaign._id}/creators/${contentData._id}/update-amount`,
        {
          amount: newAmount,
        }
      );
  ...
  const modalOfAmount = (
    ...
    >
      ...
      />
      <div className="mt-4">
        <p className="text-neutral-900 text-md mb-2">Amount (USD)</p>
        <Input
  ...
  return (
    <>
        ...
        footer={() => (
          <div className="flex justify-between font-medium">
            ...
            <span>
              $
              {creators
                .reduce((sum, creator) => sum + creator.amount || 0, 0)
                .toFixed(2)}
            </span>
          </div>
        )}
          ...
          expandedRowRender: (record) => (
            <div className="bg-neutral-50 p-4 rounded-xl shadow-sm space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm font-medium text-neutral-900">
                  {moment(record.invoice?.createdAt).format(
                    "DD/MM/YYYY HH:mm A"
                  )}
                </span>
              </div>
      ...
      <Modal
        title={
          <h2 className="text-lg font-semibold text-gray-800">View Content</h2>
        }
        ...
        <div className="space-y-4 max-h-[600px] overflow-y-auto">
          {contentData?.content
            ...
            .map((content, index) => (
              <div
                key={index}
                className="p-4 bg-gray-100 rounded-lg shadow-sm relative"
              >
                <Popconfirm
                  title={
                    <span className="text-gray-800 font-medium">
                      Are you sure you want to approve this content? It will be
                      shared on LinkedIn.
                    </span>
                  }
                  onConfirm={() => {
