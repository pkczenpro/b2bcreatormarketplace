...
export default function PostMaker({}: PostMakerProps) {
  ...
  const saveDraftToLocalStorage = async (selectedCategory) => {
    ...
    try {
      ...
      toast.success(
        "✨ Your draft has been saved! You can come back to it anytime."
      );
    } catch (error) {
  ...
  const publishToLinkedIn = async () => {
    ...
    try {
      const res = await api.post(
        `/campaigns/${selectedCampaign || "independent"}/creators/${
          userData._id
        }/submit`,
        formData,
        {
        ...
        toast.success("Post shared successfully", {
          ...
          description:
            isSelectSharingModalOpenValue === "1"
              ? "Post shared to campaign, waiting for brand approval"
        ...
        >
          {groupedDrafts ? (
            Object.entries(groupedDrafts)
              .filter(
                ([category]) =>
                  filterGroup === "All" || category === filterGroup
              )
              .map(([category, drafts]) => (
                <div key={category}>
