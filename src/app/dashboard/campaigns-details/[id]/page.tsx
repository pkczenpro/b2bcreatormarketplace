...
export default function CampaignDetails({}: CampaignDetailsProps) {
  ...
  const campaignOverview = () => {
    const contentDistribution = transformData(
      campaignAnalytics?.analytics?.contentDistribution
    );
    const contentCountByType = transformData(
      campaignAnalytics?.analytics?.contentCountByType
    );
    const statusMap = {
      pending: "Pending",
      approved: "Approved",
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-neutral-100 rounded-2xl p-6">
            ...
            <BarChartComponent
              campaignAnalytics={adjustedContentDistribution}
            />
          </div>
        ...
        <div className="flex mt-8">
          ...
          <div className="flex space-x-2 ml-4">
            {campaign?.contentTypes?.map((type: string, index: number) => (
              <span
                key={index}
                className="bg-neutral-50 text-neutral-600 border px-4 py-1 border-neutral-600 rounded-full text-text-small"
              >
                {type}
              </span>
            ))}
            ...
            >
              ...
              <div className="flex items-center mt-4">
                ...
                <div className="ml-auto">
                  ...
                  {campaign.status === "approved" && (
                    <Button
                      className="ml-auto mt-2 w-[200px]"
                      variant="primary"
                      size="small"
        ...
        <Card>
          <div className="flex flex-col items-center justify-center gap-4 w-full p-4">
            <h2 className="text-xl font-semibold text-center">
              🎉 Congratulations! <br /> You have been approved by{" "}
            ...
            <div className="flex flex-col items-center justify-center gap-3 w-full mt-4">
              <Link
                href={`/dashboard/post-maker?campaign=${id}`}
                className="w-full"
              >
                <Button
              ...
              <Link
                href={`/dashboard/carousel-maker?campaign=${id}`}
                className="w-full"
              >
                <Button
                  className="w-full flex items-center justify-center gap-2"
