...
export default function Creators() {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-4 bg-neutral-50 p-2 px-4 rounded-xl">
          ...
          />
          <h2 className="text-md font-bold text-neutral-600">{creator.name}</h2>
        </div>
        <div className="mt-4">
          <p className="text-neutral-900 text-md mb-2">Select Campaign</p>
          <Select
    ...
    if (creator) {
      try {
        ...
        toast.success("Creator added to campaign successfully.", {
          position: "top-right",
          description:
            `${creator.name} has been added to Campaign` + res.data.title,
        });
  ...
  const itemsPerPage = 30; // or any number you prefer
  ...
  return (
    <div className="flex flex-col sm:flex-row">
      ...
      <div className="flex flex-col w-full min-h-screen bg-neutral-50 px-4 py-12 overflow-y-auto max-h-screen">
        ...
        <p className="text-lg">
          Explore creators looking to collaborate and sponsor you in endless
          ways.
        </p>
        ...
        <div className="w-[40%] hidden md:block mt-4">
          <Input
            ...
            type="text"
            placeholder="Search creators and keywords..."
        ...
        <div className="flex gap-2 mt-4 flex-wrap">
          ...
          <span
            onClick={() => resetTags()}
            className="bg-primary-700 text-white text-sm px-3 py-1 rounded-full flex items-center cursor-pointer"
          >
            Reset Tags{" "}
            <RefreshCwIcon className="ml-1 w-4 h-4 inline-block text-white" />
          </span>
        </div>

        <h3 className="mt-4 text-xl font-regular mb-4">Featured Creators</h3>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {paginatedData.map((creator, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl shadow-lg p-6 space-y-4 flex flex-col justify-between"
            >
              <div>
                <div className="flex justify-between items-start">
                  <div className="flex items-center space-x-4">
                    <div className="relative w-14 h-14">
                      ...
                      {activeUsers.includes(creator._id) && (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-500 flex items-center">
                        <MapPin className="w-4 h-4 text-neutral-600 mr-1" />
                        {creator.location}
                      </p>
                  ...
                  <div className="flex items-start justify-between space-x-2">
                    ...
                    <Link
                      href={`/dashboard/user-preview/${creator._id}`}
                      className="flex items-center gap-2 text-primary-600 bg-neutral-50 rounded-full p-2"
                    >
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {creator?.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="text-neutral-600  text-sm px-3 py-1 rounded-full border border-neutral-200 bg-neutral-50"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="flex gap-2 flex-wrap">
                <Link href="/dashboard/inbox" className="w-full">
                  <Button className="w-full px-4 py-2 border border-neutral-300 rounded-lg">
                    Message
                  </Button>
                </Link>
                <Button
        ...
        <div className="flex justify-center mt-5 space-x-2 mb-3">
          {currentPage > 1 && (
            <button
